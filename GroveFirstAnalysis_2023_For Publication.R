#Heck et al. R Code for Phase I data analysis
# Load libraries ----
library(readxl)
library(dplyr)
library(tibble)
...
canopy.density <- canopy.density %>%
  select(Row, TreeID, MoleculeID, Set, starts_with("Canopy_Density"))
...
canopy.volume <- canopy.volume %>%
  select(Row, TreeID, MoleculeID, Set, TrunkDiameter_PreHarvest_mm, CanopyHeight_PreHarvest_m,
...
plant.health <- plant.health %>%
  select(Set, Row, TreeID, MoleculeID, starts_with("East"), starts_with("West"))
...
# compare Fruit counts from Sorter and Drop data
merge3 %>% select(TreeID, MoleculeID, Row, TotalSorterFruit, NumFruit) %>%
  rename(Fruit.Count.From.SorterData = NumFruit, Fruit.Count.From.DropData = TotalSorterFruit) %>%
  filter(Fruit.Count.From.SorterData != Fruit.Count.From.DropData) %>%
  View() # they don't always agree


###Figure 4 a - Pareto chart for fruit >200g
# 4a-Filter data for fruit weight > 200g and remove "Not Part of Trial"
filtered_data <- fruit.sorter %>%
  filter(Weight > 200, MoleculeID != "Not Part of Trial") %>%
  group_by(MoleculeID) %>%
  summarise(NumFruit = n(), .groups = 'drop')

# 4a-Compute total number of fruit per Molecule ID for ordering
molecule_order <- filtered_data %>%
  arrange(desc(NumFruit)) %>%
  pull(MoleculeID)

# 4a-Reorder the Molecule IDs based on total fruit count
filtered_data$MoleculeID <- factor(filtered_data$MoleculeID, levels = molecule_order)

# 4a-Define colors: Highlight M10, M53, M57, and M20; others black
...
# 4a-Define custom colors
color_palette <- c("M10" = "red", "M53" = "blue", "M57" = "green", "M20" = "purple", "Other" = "black")

# 4a-Create the stacked bar graph with custom colors
ggplot(filtered_data, aes(x = MoleculeID, y = NumFruit, fill = Color)) +
  ...
  theme(
    ...
    axis.text.x = element_text(size = 8, angle = 90, hjust = 1, vjust = 0.5),  # Rotate labels for readability
...
###Figure 4b Box plot comparing Fruit Average Weight values for selected
ggplot(fulldata %>% filter(MoleculeID %in% c("M10","M20","M57","M53")), aes(x = MoleculeID, y = AvgWeight, fill = MoleculeID)) +
  geom_boxplot() +
  labs(
    title = "Average Weight",
    x = "Treatment (MoleculeID)",
    y = "Average Weight"
  ) +
  theme_minimal()

###Figure 4c Box plot comparing boxes per acre values for selected
ggplot(fulldata %>% filter(MoleculeID %in% c("M10","M20","M57","M53")), aes(x = MoleculeID, y = Box_perAcre, fill = MoleculeID)) +
  geom_boxplot() +
  labs(
    title = "Boxes per Acre",
    x = "Treatment (MoleculeID)",
    y = "Boxes per Acre"
  ) +
  theme_minimal()

###Figure 4d Box plot comparing pounds solid per box for selected
ggplot(fulldata %>% filter(MoleculeID %in% c("M10","M20","M57","M53")), aes(x = MoleculeID, y = Solid_perBox, fill = MoleculeID)) +
  geom_boxplot() +
  labs(
    title = "Pounds Solid Per Box",
    x = "Treatment (MoleculeID)",
    y = "Lbs Solid"
  ) +
  theme_minimal()

###Figure 4e Box plot comparing Fruit Brix values for selected
...
###Figure 4f Box plot comparing TSS:TA ratio values for selected
...
###Figure 4g Box plot comparing % juice for selected
...
###Extended Data Fig. 9a Number of fruit by MID (includes sorted, dropped and skirted fruit)
## trees were skirted and # fruit on skirted branches were counted and referred to ask skirted fruit
# 9a-create treatment ID without the M
fulldata <- fulldata %>%
...
###Extended Data Fig. 9b Avg fruit weight vs number of fruit
...
###Extended Data Fig. 9c Avg fruit weight vs number of fruit without color
...
###Extended Data Fig. 1a Tree rating by row
...
### Extended Data Fig. 1b Number of fruit per row by Tree Rating w legend
...
### Extended Data Fig. 1c Avg fruit weight per row 
...
###Extended Data Fig. 7a Percent Fruit Drop by MID colored by TreeRating
...
### Extended Data Figure 4b Plant health index Day 0 to 180----
...
### Extended Data Figure 4c Plant health index Day 0 to 270----
...
###Extended Data Figure 8a Day 90 Number of Total fruit (sorted only) vs change in plant health index 
...
###Extended Data Figure 8a Day 180 Number of Total fruit (sorted only) vs change in plant health index 
...
###Extended Data Figure 8a Day 270 Number of Total fruit (sorted only) vs change in plant health index
...
###Extended Data Figure 8b Day 0 to 90 Fruit Weight x Plant Health Index
...
###Extended Data Figure 8b Day 0 to 180 Fruit Weight x Plant Health Index
...
###Extended Data Figure 8b Day 0 to 270 Fruit Weight x Plant Health Index
...
###Extended Data Figure 6a Plant health vs Tree rating Day 90
...
###Extended Data Figure 6b Plant health vs Tree rating Day 180
...
###Extended Data Figure 6c Plant health vs Tree rating Day 270
...
###Alt Figure 5 Hierarchical clustering at the molecule level with selected phenotypes
MID.variables <- fulldata %>%
  group_by(MoleculeID) %>%
  summarise_if(is.numeric, mean, na.rm = TRUE) %>%
  select(MoleculeID, AvgWeight, NumFruit, AvgBrix, TotalFruitDrop, PctFruitDrop, Canopy_Density_East_Day270,
         East_delta_270, East_270, Box_perAcre, SkirtedFruit_T1, TrunkDiameter_PreHarvest_mm, 
...
sub_grp_6 <- cutree(hc2, k = 6)

###Figure 5 Hierarchical clustering at the molecule level with selected phenotypes final
MID.variables2 <- fulldata %>%
  group_by(MoleculeID) %>%
  summarise_if(is.numeric, mean, na.rm = TRUE) %>%
  select(MoleculeID, AvgWeight, NumFruit, AvgBrix, Tree_delta_180, Tree_delta_90, TotalFruitDrop,
         CanopyVolume_PreHarvest_m3, CanopyHeight_PreHarvest_m, Solid_perBox, PctJuice) %>%
  column_to_rownames(var = "MoleculeID") 

MID.variables2.r <- cor(as.matrix(MID.variables2), use = "pairwise.complete.obs")
View(MID.variables2.r)

d2 <- dist(scale(MID.variables2), method = "euclidean")
hc2 <- hclust(d2, method = "complete")
plot(hc2, cex = 0.6, main = "Complete Linkage", sub = "", xlab = "", hang = -1)
plot(hc2, cex = 0.6, main = "Complete Linkage", sub = "", xlab = "", hang = -1)
rect.hclust(hc2, k = 10, border = 2:5)
sub_grp_6 <- cutree(hc2, k = 10)
...
phenotype.data <- phenotype.data %>%
  mutate(trmt.ind.single = "1" ) %>%
  mutate(trmt.ind.single = ifelse(MoleculeID %in% c("M53", "M10", "M1", "M55", "M58", "M68", "M66", "M65", "M63", "M62"), "2", trmt.ind.single)) %>%
  mutate(trmt.ind.separate = ifelse(MoleculeID %in% c("M53", "M10", "M1", "M55", "M58", "M68", "M66", "M65", "M63", "M62"), MoleculeID, trmt.ind.single))

###Fig2a Tree health, Fruit drop and Solids per box
ggplot(phenotype.data, aes(Tree_delta_180, TotalFruitDrop, 
                           size = Solid_perBox, color = trmt.ind.separate)) +
  geom_point(alpha = 0.6) +
  guides(size = guide_legend(title = "Pounds Solid/Box")) +
  guides(color = guide_legend(title = "Treatment")) +
  xlab("Change in Tree Health at 180 Days") +
  ylab("Fruit Drop (count)") +
  theme_bw()

# make plotted points for Trt = 1 more transparent
ggplot(phenotype.data, aes(Tree_delta_180, TotalFruitDrop, 
                           size = Solid_perBox, color = trmt.ind.separate)) +
  geom_point(aes(alpha = trmt.ind.single)) +
  guides(size = guide_legend(title = "Fruit Weight (kg)")) +
  guides(color = guide_legend(title = "Treatment")) +
  theme_bw() +
  scale_alpha_discrete(range = c(0.2, 0.8))

###Fig3a Brix, Acidity & Fruit Weight plots
ggplot(phenotype.data, aes(Brix, TSS_TA_Ratio, size = JuiceVolumeCorrected_mL,
                           color = trmt.ind.single)) +
  geom_point(alpha = 0.6)

ggplot(phenotype.data, aes(Brix, TSS_TA_Ratio, size = JuiceVolumeCorrected_mL,
                           color = trmt.ind.separate)) +
  geom_point(alpha = 0.6) +
...
phenotype.data <- phenotype.data %>%
  mutate(trmt.ind.single = "1" ) %>%
  mutate(trmt.ind.single = ifelse(MoleculeID %in% c("M15", "M12", "M20", "M16", "M29", "M34", "M36", "M41"), "2", trmt.ind.single)) %>%
  mutate(trmt.ind.separate = ifelse(MoleculeID %in% c("M15", "M12", "M20", "M16", "M29","M34", "M36", "M41"), MoleculeID, trmt.ind.single))

###Fig2b Tree health, Fruit drop and Solids per box
ggplot(phenotype.data, aes(Tree_delta_180, TotalFruitDrop, 
                           size = Solid_perBox, color = trmt.ind.separate)) +
  geom_point(alpha = 0.6) +
  guides(size = guide_legend(title = "Pounds Solid/Box")) +
...
###Fig3b Brix, Acidity & Fruit Weight plots
ggplot(phenotype.data, aes(Brix, TSS_TA_Ratio, size = JuiceVolumeCorrected_mL,
                           color = trmt.ind.single)) +
  geom_point(alpha = 0.6)
...
phenotype.data <- phenotype.data %>%
  mutate(trmt.ind.single = "1" ) %>%
  mutate(trmt.ind.single = ifelse(MoleculeID %in% c("M49", "M43", "M48", "M47", "M52", "M51", "M57", "M59", "M54", "M78"), "2", trmt.ind.single)) %>%
  mutate(trmt.ind.separate = ifelse(MoleculeID %in% c("M49", "M43", "M48", "M47", "M52", "M51", "M57", "M59", "M54", "M78"), MoleculeID, trmt.ind.single))

###Fig2c Tree health, Fruit drop and Solids per box
ggplot(phenotype.data, aes(Tree_delta_180, TotalFruitDrop, 
                           size = Solid_perBox, color = trmt.ind.separate)) +
  geom_point(alpha = 0.6) +
  guides(size = guide_legend(title = "Pounds Solid/Box")) +
...
###Fig 3c Brix, Acidity & Fruit Weight plots
ggplot(phenotype.data, aes(Brix, TSS_TA_Ratio, size = JuiceVolumeCorrected_mL,
                           color = trmt.ind.single)) +
  geom_point(alpha = 0.6)
...
phenotype.data <- phenotype.data %>%
  mutate(trmt.ind.single = "1" ) %>%
  mutate(trmt.ind.single = ifelse(MoleculeID %in% c("M13", "M14", "M42", "M46", "M56", "M67", "M70", "M61", "M71", "M83"), "2", trmt.ind.single)) %>%
  mutate(trmt.ind.separate = ifelse(MoleculeID %in% c("M13", "M14", "M42", "M46", "M56", "M67", "M70", "M61", "M71", "M83"), MoleculeID, trmt.ind.single))

###Fig2d Tree health, Fruit drop and Solids per box
ggplot(phenotype.data, aes(Tree_delta_180, TotalFruitDrop, 
                           size = Solid_perBox, color = trmt.ind.separate)) +
  geom_point(alpha = 0.6) +
  guides(size = guide_legend(title = "Pounds Solid/Box")) +
...
###Fig3d Brix, Acidity & Fruit Weight plots
ggplot(phenotype.data, aes(Brix, TSS_TA_Ratio, size = JuiceVolumeCorrected_mL,
                           color = trmt.ind.single)) +
  geom_point(alpha = 0.6)
...
phenotype.data <- phenotype.data %>%
  mutate(trmt.ind.single = "1" ) %>%
  mutate(trmt.ind.single = ifelse(MoleculeID %in% c("M53", "M10", "M20", "M42", "M54", "M47", "M75", "M73", "M49"), "2", trmt.ind.single)) %>%
  mutate(trmt.ind.separate = ifelse(MoleculeID %in% c("M53", "M10", "M20", "M42", "M54", "M47", "M75", "M73", "M49"), MoleculeID, trmt.ind.single))

###Fig2e Tree health, Fruit drop and Solids per box
ggplot(phenotype.data, aes(Tree_delta_180, TotalFruitDrop, 
                           size = Solid_perBox, color = trmt.ind.separate)) +
  geom_point(alpha = 0.6) +
  guides(size = guide_legend(title = "Pounds Solid/Box")) +
...
###Fig3e Brix, Acidity & Fruit Weight plots
ggplot(phenotype.data, aes(Brix, TSS_TA_Ratio, size = JuiceVolumeCorrected_mL,
                           color = trmt.ind.single)) +
  geom_point(alpha = 0.6)
...
###Extended Data Table 2 Pairwise t-tests for fruit drop between each Molecule and M53 assuming unequal variances
...
FruitDrop_tests <- fulldata %>%
  ...
  select(MoleculeID, avg_FruitDrop, avg_FruitDrop_M53, delta, p_value) %>%
  arrange(p_value) %>%
  View()

###Extended Data Table 3 Pairwise t-tests for yield between treatments and M53 assuming unequal variances
# Welch's ANOVA (doesn't assume equal variances)
oneway.test(TotalFruit ~ MoleculeID, var.equal = F, data = fulldata)

# Pairwise t-tests between each Molecule and M53 assuming unequal variances
...
TotalFruit_tests <- fulldata %>%
  ...
  select(MoleculeID, avg_TotalFruit, avg_TotalFruit_M53, delta, p_value) %>%
  arrange(p_value) %>%
  View()

###Extended Data Table 4 Pairwise t-tests for fruit weight between each molecule and M53 assuming unequal variances
# Welch's ANOVA (doesn't assume equal variances)
...
FruitWeight_tests <- fulldata %>%
  ...
  select(MoleculeID, avg_FruitWeight, avg_FruitWeight_M53, delta, p_value) %>%
  arrange(p_value) %>%
  View()

###Extended Data Table 5 Pairwise t-tests for Brix between each Treatment and M53 assuming unequal variances
# Welch's ANOVA (doesn't assume equal variances)
...
Brix_tests <- fulldata %>%
  ...
  select(MoleculeID, avg_Brix, avg_Brix_M53, delta, p_value) %>%
  arrange(p_value) %>%
  View()

###Extended Data Table 6 Pairwise t-test comparison for Solids per box assuming unequal variances
...
Solid_perBox_tests <- fulldata %>%
  ...
  select(MoleculeID, avg_Solid_perBox, avg_Solid_perBox_M53, delta, p_value) %>%
  arrange(p_value) %>%
  View()
